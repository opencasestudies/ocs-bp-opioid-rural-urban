...
```{r}
d0 <- dfAnnual  %>% group_by(STATE,YEAR) %>%
     summarise( mean_DOSAGE = mean(DOSAGE_UNIT))

p0 <- ggplot(d0, aes(x = YEAR, y = mean_DOSAGE, color = STATE)) + 
  geom_line_interactive(aes(group= STATE, 
  data_id = abbr2state(STATE), tooltip = abbr2state(STATE) ), 
  show.legend = FALSE)+
  labs(title = "Mean Dosage Across State")+
  theme_minimal()

girafe(code = print(p0) )
```

State California has larger amount of dosage than others while
...
## Across State with Denstiy Normalizaiton

```{r, fig.width=5, fig.height = 6}
d1 <- df_dens  %>% group_by(STATE,YEAR) %>%
     summarise( mean_DOSAGE = mean(dens_DOSAGE))

p8 <- ggplot(d1, aes(x = YEAR, y = mean_DOSAGE, color = STATE)) + 
  geom_line_interactive(aes(group= STATE, 
  data_id = abbr2state(STATE), tooltip = abbr2state(STATE) ), 
  show.legend = FALSE)+
  labs(title = "Mean dens_Dosage Across State")+
  theme_minimal()

girafe(code = print(p8) )
```


Mean dens_dosage in Florida are obviously the highest, and that of 
District of Columbia is the lowest! Different 
from result without normalization.
...
getMonthDrug <- function(s){
  ...
  group_by(year,month) %>%
  summarise( mean_DOSAGE = mean(DOSAGE_UNIT))%>%
  mutate(id = as.Date(paste(year,as.character(month),'01',sep='-') ),
         STATE=s)%>% 
...
ggplot(dfState , aes(x = id, y = mean_DOSAGE)) + 
  ...
  theme_minimal()+
  scale_x_date(date_minor_breaks = "2 years")

```


Next step is Autoregression??
