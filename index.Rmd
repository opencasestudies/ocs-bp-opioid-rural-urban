...
#### {.outline }
```{r, echo=FALSE}
knitr::include_graphics(here("www", "img",
                             "mainplot.png"))
```
...
include_graphics(here::here("www", "img", "introducedbyprescription.png"))
```
...
include_graphics(here::here("www", "img", "james-yarema-5tyMgag0wRo-unsplash.jpg"))
```
...
[dplyr](https://dplyr.tidyverse.org/){target="_blank"}      | to filter, subset, join, and modify and summarize the data (also provides the `%>%` pipe operator)   
...
```{r, echo = FALSE}
knitr::include_graphics(here::here("www", "img","ingo-doerrie-Ti6Sk5rZRP8-unsplash.jpg" ))
...
knitr::include_graphics(here::here("www", "img","Opium_pod_cut_to_demonstrate_fluid_extraction1.jpg" ))
```
...
```{r, echo = FALSE}
knitr::include_graphics(here::here("www", "img", "Opioids_Infographic.png"))
```
...
```{r, echo = FALSE}
knitr::include_graphics(here::here("www", "img", "2018-3-Wave-Lines-Mortality.png"))
...
For the purposes of this case study, we have saved files after getting the data from the API that you may import into your R environment using the `OCSdata` package and the `load()` function:

```{r, eval=FALSE}
# install.packages("OCSdata")
# library(OCSdata)
imported_data("ocs-bp-opioid-rural-urban", outpath = getwd())
load(here::here("OCS_data", "data", "imported", "land_area.rda"))
load(here::here("OCS_data", "data", "imported", "county_pop_arcos.rda"))
load(here::here("OCS_data", "data", "imported", "county_annual.rda"))
```
...
```{r, eval=FALSE}
land <- readxl::read_excel(here::here("OCS_data", "data", "raw", "LND01.xls"))
land <- as_tibble(land)
```

```{r, echo=FALSE}
land <- readxl::read_excel(here::here("www", "data", "raw", "LND01.xls"))
land <- as_tibble(land)
```
...
write.csv(land, file = here::here("OCS_data", "data", "simpler_import", "land_area.csv"))
```

```{r, eval = FALSE}
save(land, file =  here::here("OCS_data", "data", "imported", "land_area.rda"))
```
...
We are interested in the county level data. The original API included county-level population data (the number of people per county in each state in the US from 2006 to 2014).
...
```{r, echo=FALSE}
knitr::include_graphics(here::here("www", "img", "get.png"))
...
```{r, echo=FALSE}
knitr::include_graphics(here::here("www", "img", "tryitout.png"))
```

3. Entering the key.

```{r, echo=FALSE}
knitr::include_graphics(here::here("www", "img", "key.png"))
```

4. Clicking the "Execute" button.

```{r, echo=FALSE}
knitr::include_graphics(here::here("www", "img", "execute.png"))
```
...
We could have copied the URL section `"https://arcos-api.ext.nile.works/v1/county_population?key=uO4EK6I"` and used it in the `GET()` function of the `httr` package :
...
The object called `county_pop_json` would have been a `json` object. A [JavaScript Object Notation (JSON)](https://fileinfo.com/extension/json#:~:text=A%20JSON%20file%20is%20a,web%20application%20and%20a%20server.) object (or file format) is [lightweight](https://en.wikipedia.org/wiki/Lightweight_programming_language) meaning that it does not take up much memory and they are human readable files to make transmitting data from websites easier.

You would also see that the `Status` is `200`, which means that you were successful in retrieving the data from the API.

We would then use the `content()` function of the `httr` package to extract the text from the file:
...
This would be a very large string at this point. We could take a look at part of it by using the `str_sub()` function of the `stringr` package. In this case we would only look at the first 400 characters.
...
To get the data into a more readable format, we would use the `fromJSON()` function of the `jsonlite` package and again create a tibble of the data using `as_tibble()`
...
We could use the `glimpse()` function and the `distinct()` function of the `dplyr` package to get a better sense of the data. The `distinct()` function allows us to take a look at the unique values of the `year` variable.
...
```{r}
load(here::here("www", "data", "imported", "county_pop_arcos.rda"))
```
...
<details> <summary> Click here to reveal what the code would have looked like. </summary>
...
```{r}
load(here::here("www", "data", "imported", "county_annual.rda"))
```
...
write.csv(monthlyDosage, 
          file = here::here("OCS_data", "data","extra", "county_monthly.csv"))

save(monthlyDosage, 
     file =  here::here("OCS_data", "data", "extra", "county_monthly.rda"))
```
...
```{r, echo=FALSE}
load(here::here("www", "data", "imported", "land_area.rda"))
load(here::here("www", "data", "imported", "county_pop_arcos.rda"))
load(here::here("www", "data", "imported", "county_annual.rda"))
```
...
We will also start using the `%>%` pipe operator for our assignments.

***
<details> <summary> Click here if you are unfamiliar with piping in R, which uses this `%>%` operator</summary>  


By [piping](https://r4ds.had.co.nz/pipes.html){target="_blank"} we mean using the `%>%` pipe operator, which is available after loading `dplyr` or the `tidyverse`. 
...
Suppose we have a data set called `mydata`. Using functions that we talked about earlier, check if there are missing data for the variables. If so, how would you check which data are missing?
...
We use the standard assignment operator `<-` combined with the `%>%` pipe to update `annualDosage` in place. This passes `annualDosage` into the pipeline on the right-hand side and assigns the result back to the same variable name, avoiding the creation of an intermediate tibble.
...
```{r, echo=FALSE}
save(annualDosage, file = here("www", "exercise", "dw_code1.rda"))
```
...
We will now combine the `annualDosage` data with the `county_info` tibble.
...
```{r, echo=FALSE}
save(annualDosage, county_info, file = here("www", "exercise", "dw_code2.rda"))
```
...
# To do so, we first need to make the "year" variable a factor as it is in the `county_info` tibble
annualDosage <- annualDosage %>%
...
```{r, echo=FALSE}
save(Annual, file = here("www", "exercise", "dw_code3.rda"))
```
...
```{r, eval=FALSE}
write.csv(Annual, file = here::here("OCS_data", "data","wrangled", "Annual_opioid_data.csv"))
save(Annual, file =  here::here("OCS_data", "data","wrangled", "Annual_opioid_data.rda"))
write.csv(county_info, file = here::here("OCS_data", "data", "wrangled", "county_info.csv"))
save(county_info, file = here::here("OCS_data", "data", "wrangled", "county_info.rda"))
```

```{r, echo=FALSE}
write.csv(Annual, file = here::here("www", "data","wrangled", "Annual_opioid_data.csv"))
save(Annual, file =  here::here("www", "data","wrangled", "Annual_opioid_data.rda"))
write.csv(county_info, file = here::here("www", "data", "wrangled", "county_info.csv"))
save(county_info, file = here::here("www", "data", "wrangled", "county_info.rda"))
```
...
quiz(caption = "",
  question("If we want to modify a tibble in place using a sequence of `%>%` pipe operations, which of the following is the correct approach in dplyr?",
    answer("`df <- df %>% mutate(...)`", correct = TRUE, message = "Correct! Using `<-` to assign the result of a pipeline back to the same variable name is the standard dplyr approach."),
    answer("`df %<>% mutate(...)`", message = "The `%<>%` compound assignment pipe from magrittr works, but using explicit `<-` assignment with `%>%` is clearer and more standard."),
    answer("`df %>% mutate(...)`", message = "This pipes into `mutate()` but does not save the result anywhere."),
    answer("`mutate(df, ...)`", message = "This returns a modified tibble but does not update `df` unless assigned with `<-`."),
    allow_retry = TRUE,
  ...
  question("Which of the following logical operators indicates logical OR in R?",
    answer("`&`", message = "`&` is the logical AND operator."),
    answer("`!`", message = "`!` is the logical NOT operator."),
    answer("`|`", correct = TRUE, message = "Note both `|` and `||` are logical OR operators, but they are different. See the [documentation](https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/Logic) for details."),
    allow_retry = TRUE,
    random_answer_order = TRUE
  )
)
...
1. Convert row names to a column called `car`. (The code is given in the first hint.) 
2. Only keep the `car`, `mpg`, and `cyl` variables of the data.
3. Create a new variable called `mpg_group`, whose value is "low" when `mpg` is lower than or equal to 16.7 (the first tercile of the data), "middle" when it is greater than 16.7 (the first tercile) and less than or equal to 21.4 (the second tercile), and "high" when it is greater than 21.4 (the second tercile).

The terciles are found using the following command:

```{r}
quantile(mtcars$mpg, probs = seq(0, 1, 1/3))
```
...
```{r dw_exercise1, exercise=TRUE, eval=FALSE}
# Note there might be other ways to do this that give us the same results 
# For practice purposes try to fill in the blanks
mtcars %>%
  # step 1
  tibble::rownames_to_column("___") %>%
  # step 2
  dplyr::______(___, ___, ___) %>%
  # step 3
  dplyr::mutate(mpg_group = _________(___ __ 16.7 ~ "___", 
                                      ___ _ 16.7 _ ___ __  21.4  ~ "______",
                                      ___ _ 21.4 ~ "____"))
```

```{r dw_exercise1-hint-1}
mtcars %>%
  # step 1
  tibble::rownames_to_column("car")
```

```{r dw_exercise1-hint-2}
mtcars %>%
  # step 1
  tibble::rownames_to_column("car") %>%
  # step 2
  dplyr::select(car, mpg, cyl)
```

```{r dw_exercise1-hint-3}
...
mtcars %>%
  ...
  dplyr::select(car, mpg, cyl) %>%
  # step 3
  dplyr::mutate(mpg_group = case_when(mpg <= 16.7 ~ "low", 
                                      mpg > 16.7 & mpg <=  21.4  ~ "middle",
                                      mpg >  21.4 ~ "high"))
  
# an alternative way for step 3 by directly using the quantile function
  # dplyr::mutate(mpg_group = case_when(mpg <= quantile(mtcars$mpg, probs = 1/3) ~ "low", 
  #                                     mpg > quantile(mtcars$mpg, probs = 1/3) & 
  #                                       mpg <=  quantile(mtcars$mpg, probs = 2/3) ~ "middle",
  #                                     mpg >  quantile(mtcars$mpg, probs = 2/3) ~ "high"))
...
```{r, echo=FALSE}
load(file = here::here("www", "data","wrangled", "Annual_opioid_data.rda"))
load(file = here::here("www", "data", "wrangled", "county_info.rda"))
```
...
```{r, echo = FALSE, out.width = "60%"}
knitr::include_graphics(here::here("www", "img", "geom.png"))
```
...
```{r, echo = FALSE, out.width = "60%"}
knitr::include_graphics(here::here("www", "img", "theme.png"))
```
...
```{r, echo=FALSE}
save(Annual, file = here("www", "exercise", "dv_code1.rda"))
```
...
```{r dv_code1, exercise=TRUE, exercise.lines = 15}
Annual %>%
```
...
If you completed the exercise, you would get a plot like this:
...
```{r, echo=FALSE}
save(Annual, file = here("www", "exercise", "dv_code2.rda"))
```

```{r dv_code2-setup}
library(tidyverse)

load(here("www", "exercise", "dv_code2.rda"))
```

```{r dv_code2, exercise=TRUE, exercise.lines = 15}
Annual %>%
```
...
```{r, echo=FALSE}
save(Annual, raw_state_avg, file = here("www", "exercise", "dv_code3.rda"))
```
...
load(here("www", "exercise", "dv_code3.rda"))
```

```{r dv_code3, exercise=TRUE, exercise.lines = 25}
# create a plot of the average for normalized data
norm_state_avg <-
  Annual %>% 
...
```{r, echo=FALSE}
save(Annual, file = here("www", "exercise", "dv_code4.rda"))
```
...
load(here("www", "exercise", "dv_code4.rda"))
```

```{r dv_code4, exercise=TRUE, exercise.lines = 20}
# Try this out
```
...
```{r, echo=FALSE}
save(Annual, file = here("www", "exercise", "dv_code5.rda"))
```
...
load(here("www", "exercise", "dv_code5.rda"))
```

```{r dv_code5, exercise=TRUE, exercise.lines = 20}
# Try this out
```
...
If you completed the exercise, you would get a table like this:
...
question("Which of the following is the long format of the data?",
    answer("A", message = "This is the wide format because each variable has its own column."),
...
question("Which one of the following inside the `ggplot()` function can be used to define the shape, transparency, color, fill, and line types of the plot?",
...
```{r, echo=FALSE}
include_graphics(here::here("www", "img", "null.png"))
```
