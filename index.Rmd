...
Note that if you have trouble with the data import from the API, we have saved files after getting the data from the API that you may import into your R environment using the `OCSdata` package:
...
annual_url <- "https://arcos-api.ext.nile.works/v1/combined_county_annual?key=uO4EK6I"
```
...
# put the data into a more readable format
county_annual <-
...
If you have been following along but stopped, we could load our wrangled data using the `OCSdata` package like so:
...
Then, you may download and load the wrangled data into your R environment using the following function:
