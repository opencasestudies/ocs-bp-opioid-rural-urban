...
You can see that on average 10 pills were shiped for each person in the first row.
...
Let's see how this changes the state specific data.
...
```{r}
t.test(DOSAGE_UNIT ~ rural_urban, data = Annual, var.equal = FALSE)
t.test(DOSAGE_UNIT ~ large_urban, data = Annual, var.equal = FALSE)

t.test(pop_DOSAGE ~ rural_urban, data = Annual, var.equal = FALSE)
t.test(pop_DOSAGE ~ large_urban, data = Annual, var.equal = FALSE)
