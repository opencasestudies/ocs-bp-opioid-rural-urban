...
# These packages are used only for the optional reading-metrics tables below.
# Do not install packages while rendering: GitHub's unauthenticated API limit
# makes an otherwise reproducible render fail intermittently.
has_reading_metrics <- requireNamespace("wordcountaddin", quietly = TRUE) &&
  requireNamespace("read.so", quietly = TRUE)
...
```{r, echo=FALSE}
if (has_reading_metrics) {
  readtable = wordcountaddin::text_stats("index.Rmd") # producing reading time markdown table
  ...
  cat("Reading-time metrics are unavailable because the optional `wordcountaddin` and `read.so` packages are not installed.\\n")
}
```
...
if (has_reading_metrics) {
  ...
  cat("Readability metrics are unavailable because the optional `wordcountaddin` and `read.so` packages are not installed.\\n")
}
```
...
This article explores rates of opioid prescriptions in rural and urban communities in the United States using the [Athenahealth electronic health record (EHR) system](https://www.athenahealth.com/solutions/athenaone) for 31,422 primary care providers from January 2014 to March 2017.
...
When this case study renders in a new environment, the following chunk installs any missing packages before they are loaded.

```{r, eval=TRUE}
cran_packages <- c(
  "naniar", "formattable", "Hmisc", "ggiraph", "patchwork",
  "directlabels", "usdata", "OCSdata", "wordcountaddin", "read.so"
)
...
# Install remotes only when ggpol is not already available.
...
[ggpol](https://github.com/erocoar/ggpol) | to create plots that have jitter and half boxplots   
...
According to the [CDC](https://www.cdc.gov/overdose-prevention/about/understanding-the-opioid-overdose-epidemic.html), there were 3 waves of the epidemic:
...
Their [website](https://www.deadiversion.usdoj.gov/arcos/arcos.html) indicates that: 
...
As this is a very large dataset, thus the Washington Post created an [application programming interface (API)](https://en.wikipedia.org/wiki/API)  to make it easier for users to access the data. 
...
This [repository](https://github.com/wpinvestigative/arcos) contains the Washington Post ARCOS API client and documentation. 
...
The ARCOS API is publicly documented by the [`arcos` package](https://github.com/wpinvestigative/arcos). The examples below use its documented `WaPo` key; the rendered case study uses a bundled data snapshot so it does not depend on an external service.
...
The original ARCOS API hostname used in this case study is no longer available. The maintained `arcos` package documents the current API at [ne.washingtonpost.com/arcos](https://ne.washingtonpost.com/arcos/__docs__/). Because the service is external and can be temporarily unavailable, the website renders from a bundled snapshot of the API response. You can still use the following request code interactively when the service is available:

`curl -X GET "https://ne.washingtonpost.com/arcos/v1/county_population?key=WaPo" -H "accept: application/json"`

```{r, eval = FALSE}
count_url <- "https://ne.washingtonpost.com/arcos/v1/county_population?key=WaPo"
...
When run interactively, `county_pop_json` is a `json` object.
...
You will also see that the `Status` is `200`, which means that the data were retrieved from the API.
...
For the rendered case study, load the bundled snapshot in place of the live response:
...
```
curl -X GET "https://ne.washingtonpost.com/arcos/v1/combined_county_annual?key=WaPo"
...
```{r}
annual_url <- 
  "https://ne.washingtonpost.com/arcos/v1/combined_county_annual?key=WaPo"
```
...
countyjson <- 
  httr::GET(url = "https://ne.washingtonpost.com/arcos/v1/combined_county_monthly?key=WaPo")
...
According to the [Economic Research Service of the US Department of Agriculture (USDA)](https://www.ers.usda.gov/topics/rural-economy-population/rural-classifications/what-is-rural.aspx#:~:text=Small%20population%20size%20typically%20characterizes,50%2C000%2C%20depending%20on%20the%20definition):
...
[Cheatsheet on RStudio IDE](https://rstudio.github.io/cheatsheets/rstudio-ide.pdf){target="_blank"}  
...
[Lightweight programming languages](https://en.wikipedia.org/wiki/Lightweight_programming_language)   
...
[Bootstrapping](https://en.wikipedia.org/wiki/Bootstrapping_(statistics))    [Resampling](https://en.wikipedia.org/wiki/Resampling_%28statistics%29)
...
The source data for this case study are documented at the [ARCOS API](https://ne.washingtonpost.com/arcos/__docs__/); a snapshot is included with this project for reproducible builds.
...
[formattable](https://cran.r-project.org/web/packages/formattable/formattable.pdf) | to create a formatted table
[forcats](https://forcats.tidyverse.org/){target="_blank"}      | to reorder factor for plot
[ggpol](https://github.com/erocoar/ggpol) | to create plots that are have jitter and half boxplots   
[ggiraph](https://cran.r-project.org/web/packages/ggiraph/ggiraph.pdf)   | to create interactive plots
[patchwork](https://cran.r-project.org/web/packages/patchwork/patchwork.pdf) | to combine plots
[directlabels](https://cran.r-project.org/web/packages/directlabels/directlabels.pdf) | to add labels directly on lines within plots
[usdata](https://cran.r-project.org/web/packages/usdata/usdata.pdf) | to add full state names to plots based on the state abbreviations
