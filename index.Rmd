...
quiz(caption = "",
  ...
  question("Which of the following logical operators indicate logical OR in R?",
...
3. Create a new variable called `mpg_group`, whose value is "low" when `mpg` is lower than or equal to the first tercile (3-quantile) of the data, "middle" when it is greater than the first and and less than or equal to the second tercile, and "high" when it is greater than the second tercile. (The second hint shows you how to find the terciles.)
...
mtcars %>%
  ...
  # step 3
  dplyr::mutate(mpg_group = _________(________________________________________ ~ "___", 
...
# first tercile is
quantile(mtcars$mpg, probs = 1/3)

# second tercile is
quantile(mtcars$mpg, probs = 2/3)
```
...
```{r, echo=FALSE}
# this rda file can be used in data visualization coding exercises 1, 2, 4, and 5
save(Annual, file = here("www", "exercise", "dv_code1245.rda"))
```
...
load(here("www", "exercise", "dv_code1245.rda"))
```

```{r dv_code1, exercise=TRUE, exercise.lines = 10}
Annual %>%
```
...
```{r dv_code2, exercise=TRUE, exercise.lines = 10}
Annual %>%
```
...
```{r dv_code3, exercise=TRUE, exercise.lines = 10}
norm_state_avg <-
  Annual %>% 
...
```{r dv_code4, exercise=TRUE, exercise.lines = 10}
# Try this out
```
...
```{r dv_code5, exercise=TRUE, exercise.lines = 10}
# Try this out
```
